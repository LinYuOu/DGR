...
class DeepGlobalRegistration:
  ...
  def preprocess(self, pcd):
    ...
    # Voxelization:
    # Voxelization（体素化）是将连续的空间点云数据转换为离散的体素格子的
    # 过程。在三维空间中，体素化将连续的点云数据划分为固定大小的体素（或称为三维像素），每个体素代表一个离散的空间区域。
    # Maintain double type for xyz to improve numerical accuracy in quantization
    # 稀疏量化是将连续的坐标数据映射到离散的体素网格的过程。通过将坐标除以体素大小并应用稀疏量化算法
    #，可以将连续的坐标映射到体素网格的离散位置上。这样可以有效地压缩坐标数据，并减少存储和计算的开销。
